    ...
    .map(mod => {
        describe(mod.name, () => {
            it('implementation', async () => {
                ...
                await store.mutation({
                    delete: { todos: {} },
                    merge: { todos: { todo_01: "feed the barracuda", todo_02: "pet the hammerhead" } }
                })
                expect(store.query({})).toEqual({ todos: { todo_01: "feed the barracuda", todo_02: "pet the hammerhead" } })
                expect(store.query({ todos: { todo_01: {} } })).toEqual({ todos: { todo_01: "feed the barracuda" } })
                expect(store.query({ todos: { todo_03: { created: {} } } })).toEqual({ todos: { todo_03: { creaded: undefined } } })

                expect(store.query_path(['todos', 'todo_01'])).toEqual('feed the barracuda')
